...
public abstract class AMShutdownController {
  ...
  public void preAddHistoryEvent(DAGHistoryEvent event) {
  ...
  public void postAddHistoryEvent(DAGHistoryEvent event) {
    if (shouldShutdownPostEvent(event, historyEvents)) {
      System.exit(1);
    }
  }

  protected abstract boolean shouldShutdownPreEvent(DAGHistoryEvent event,
      List<DAGHistoryEvent> historyEvents);

  protected abstract boolean shouldShutdownPostEvent(DAGHistoryEvent event,
      List<DAGHistoryEvent> historyEvents);
}
